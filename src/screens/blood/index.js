...
class Labs extends PureComponent {
  ...
  get_api_data() {
    ...
    Services.post('get_blood', {}, payload, true)
      .then(responseJson => {
        console.log('get_blood', responseJson);
  ...
  _getdata = () => {
    ...
    return data.filter(item => {
      var itemName = item.blood_name.toLowerCase();
        ...
        <View style={{flex: 1, paddingLeft: 10, paddingRight: 5}}>
          ...
          <View style={{flexDirection: 'row'}}>
            <View
              ...
              <TouchableOpacity
                onPress={() => {
                  Linking.openURL(`tel:${item.blood_contact}`);
                }}>
            ...
            {/* <Text style={{fontSize: 10}}>Area : {item.blood_area}</Text> */}
          </View>
          <View
            style={{
              flexDirection: 'row',
  ...
  onPressDirection(item) {
    ...
    const latLng = `${item.blood_lat},${item.blood_long}`;
